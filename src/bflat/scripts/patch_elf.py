...
def find_fn_boundaries(args):
    with open(args.input_file, "rb") as f:
        ...
        p = subprocess.run(["riscv64-linux-gnu-objdump",
                            "-Cd",
                            args.input_file],
                           capture_output=True,
                           text=True,
        ...
        def flush_disasm_block():
            ...
            step = (addrs_in_block[-1] - addrs_in_block[-2]) if len(addrs_in_block) >= 2 else 4
            end  = last + max(step, 2)
        ...
        merged = {}
        starts = set(eh_sizes.keys()) | set(sym_sizes.keys())
        for s in starts:
            sz_candidates = []
            if s in eh_sizes:
                merged[s] = eh_sizes[s]
        ...
        funcs = sorted(((s, merged[s]) for s in merged if merged[s] > 0),
                       key=lambda x: x[0])
...
def find_gaps(text, funcs):
    ...
    for i,(start,size) in enumerate(funcs):
        end = start + size if size > 0 else (funcs[i+1][0] if i+1 < len(funcs) else text_end)
...
if args.split_code_data:
    ...
    fn_boundaries = find_fn_boundaries(args)
